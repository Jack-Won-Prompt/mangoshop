#!/usr/bin/env bash
#
# Mangoshop 운영 배포 스크립트 (git pull 방식)
# ---------------------------------------------------------------------------
# 사용법:
#   bash deploy.sh
#
...
#   SEED_PRODUCTS=1    1 이면 상품 시더(ImportedProductSeeder) 반영 (기본 1: idempotent 라 안전)
#
# ※ 최초 배포(폴더 없음/클론)는 이 스크립트가 아니라 초기 설치 절차를 따르세요.
#   (git clone → composer install --no-dev → .env 작성 → key:generate
...
# public/storage 가 없을 때만 스토리지 링크 생성(이미 있으면 건너뜀)
if [ ! -e public/storage ]; then
    log "스토리지 링크 생성"
    "$PHP" artisan storage:link || true
fi

# 상품 시더 반영(코드 기준 updateOrCreate → 재실행해도 중복 없이 갱신)
if [ "$SEED_PRODUCTS" = "1" ]; then
    log "상품 시더 반영 (ImportedProductSeeder · idempotent)"
    "$PHP" artisan db:seed --class=ImportedProductSeeder --force
fi
...
log "배포 상태 점검"
# 전용 점검 명령으로 확인(구버전 체크아웃 등 명령이 없을 때만 부팅 확인으로 대체)
if "$PHP" artisan list --raw 2>/dev/null | grep -q '^mangoshop:deploy-check'; then
    "$PHP" artisan mangoshop:deploy-check || die "배포가 덜 끝났습니다. 위 점검 결과대로 처리한 뒤 다시 실행하세요."
else
    "$PHP" artisan about >/dev/null 2>&1 || die "앱 부팅 실패 — 캐시/마이그레이션/.env 를 확인한 뒤 다시 실행하세요."
fi
