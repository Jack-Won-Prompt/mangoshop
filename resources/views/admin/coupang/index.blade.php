...
@if($keyword !== '')
    <div class="adm-card">
        <div class="h">
            ...
            @if($refPrice)<span class="muted" style="font-size:13px">망고샵 판매가 <b style="color:var(--a-navy)">{{ number_format($refPrice) }}원</b></span>@endif
        </div>
        ...
        @if($refPrice && $stats)
            <div style="padding:14px 20px;border-top:1px solid var(--a-line);font-size:13.5px;color:#6b7794">
                @if($refPrice <= $stats['min'])
                    ✅ 망고샵 판매가가 <b style="color:#16a34a">최저가 수준</b>입니다.
                @elseif($refPrice <= $stats['avg'])
                    망고샵 판매가가 평균 이하입니다. 최저가({{ number_format($stats['min']) }}원)보다 {{ number_format($refPrice-$stats['min']) }}원 높습니다.
                @else
                    ⚠️ 망고샵 판매가가 <b style="color:#e0322d">평균보다 높습니다</b>. 가격 조정 검토가 필요할 수 있습니다.
                @endif
            </div>
        @endif
    </div>
