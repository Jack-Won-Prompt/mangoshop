        ...
        <div class="adm-card">
            ...
            <table class="atable">
                ...
                @foreach($order->items as $it)
                    <tr><td>{{ $it->display_name }}</td><td>{{ number_format($it->price) }}원</td><td>{{ $it->quantity }}</td><td><b>{{ number_format($it->subtotal) }}원</b></td></tr>
                @endforeach
                </tbody>
            </table>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;align-items:start;margin-top:20px">
    ...
    <div class="adm-card">
        ...
        <div style="padding:20px">
            ...
            @if(! $business)
                <p class="muted" style="font-size:13.5px">사업자 회원 주문만 발행 가능합니다. (사업자등록번호 필요)</p>
            @else
                @forelse($tis as $ti)
