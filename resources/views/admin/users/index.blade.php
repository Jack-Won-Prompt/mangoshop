...
<div class="toolbar">
    <div class="filter-tabs">
        ...
        <a href="{{ route('admin.users.index', ['filter'=>'business']) }}" class="{{ request('filter')==='business' ? 'on' : '' }}">도매회원</a>
...
<div class="adm-card">
    <table class="atable">
        <thead><tr><th>이름</th><th>이메일</th><th>구분</th><th>상호</th><th>승인</th><th>적립금</th><th>가입일</th><th></th></tr></thead>
        <tbody>
        @forelse($users as $u)
            <tr>
                <td><b>{{ $u->name }}</b> @if($u->is_admin)<span class="pill pill-b">관리자</span>@endif</td>
                <td>{{ $u->email }}</td>
                <td>{{ $u->member_type==='business' ? '도매' : '일반' }}</td>
