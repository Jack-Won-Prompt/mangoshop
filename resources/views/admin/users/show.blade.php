...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;align-items:start">
    <div class="adm-card">
        ...
        <div style="padding:18px 20px;font-size:14px;line-height:2.1">
            ...
            <b>회원구분</b> {{ $user->member_type==='business' ? '도매 회원' : '일반 회원' }}<br>
        ...
        <div style="padding:20px">
            @if($user->member_type==='business')
                <div style="background:#f7f9fc;border-radius:10px;padding:14px;margin-bottom:16px;font-size:13.5px;line-height:1.9">
                    <b>상호</b> {{ $user->company_name }}<br>
                    <b>사업자번호</b> {{ $user->biz_no }}<br>
                    <b>종별</b> {{ $user->biz_type ?? '-' }}
                </div>
            @else
                <p class="muted" style="margin-bottom:16px;font-size:13.5px">일반 회원입니다. 도매 승인 및 전용가는 도매 회원에만 적용됩니다.</p>
            @endif
            ...
            <form method="POST" action="{{ route('admin.users.approve', $user) }}">
                ...
                <div class="afield">
                    ...
                    <div class="ahint">'승인' 도매 회원에게 아래 도매 전용가가 적용됩니다.</div>
                </div>
            ...
            <form method="POST" action="{{ route('admin.users.update', $user) }}">
                ...
                <div class="afield">
                    ...
                    <select name="member_type" class="aselect">
                        ...
                        <option value="business" {{ $user->member_type==='business'?'selected':'' }}>도매</option>
                    </select>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
                    <div class="afield" style="margin:0"><label>상호</label><input type="text" name="company_name" class="ainput" value="{{ old('company_name', $user->company_name) }}"></div>
...
<div class="adm-card" style="margin-top:20px">
    <div class="h">
        <span>도매 전용가 매핑 <span class="pill pill-b">{{ $prices->count() }}개 제품</span></span>
    ...
    <div style="padding:20px">
        ...
        <form method="POST" action="{{ route('admin.users.prices.store', $user) }}" style="display:flex;gap:10px;align-items:flex-end;margin-bottom:18px;flex-wrap:wrap">
            ...
            <div class="afield" style="width:160px;margin:0">
                <label>도매 전용가(원)</label>
                <input type="number" name="price" class="ainput" min="0" required>
            </div>
            <button class="abtn abtn-pri">전용가 등록/수정</button>
        </form>
        <div class="ahint" style="margin:-8px 0 16px">같은 제품을 다시 등록하면 가격이 갱신됩니다. 매핑되지 않은 제품은 기본 도매가(없으면 정가)로 판매됩니다.</div>

        {{-- 목록 --}}
        <table class="atable">
            <thead><tr><th>제품</th><th>상품코드</th><th style="text-align:right">정가</th><th style="text-align:right">도매 전용가</th><th style="text-align:right">할인율</th><th></th></tr></thead>
            <tbody>
            ...
            @empty
                <tr><td colspan="6" style="text-align:center;color:#97a0b8;padding:30px">등록된 도매 전용가가 없습니다.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
